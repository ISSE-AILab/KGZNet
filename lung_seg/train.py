...
def run(model, train_loader, val_loader, criterion, vis):
    ...
    for epoch in range(1, opt.epochs+1):
        ...
        if avg_iou > best_iou:
            best_iou = avg_iou
            if opt.save_model:
                save_path = './checkpoints/my_UNet_' + str(round(best_iou, 3)) + '_' + time.strftime('%m%d%H%M')
...
if __name__ == '__main__':
    ...
    if opt.is_train:
        ...
        if opt.is_cuda:
            ...
            if opt.n_gpu > 1:
                model = nn.DataParallel(model)
        test_loader = get_test_loader(batch_size=1, shuffle=True,
                                      num_workers=opt.num_workers,
                                      pin_memory=opt.pin_memory)
        # load the model and run test
        model.load_state_dict(torch.load(os.path.join(opt.checkpoint_dir, '/')))
